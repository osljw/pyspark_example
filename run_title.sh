...
if [[ $run_mode == "local" ]]; then
    title_files=file://$WK_DIR/../${track}/${track}_title.txt
    output_dir=file://$WK_DIR/../${track}/${track}_title
    rm -r ${output_dir:7} || true
else
    title_files=<hdfs_path>/${track}/${track}_title.txt
    output_dir=<hdfs_path>/${track}/${track}_title
